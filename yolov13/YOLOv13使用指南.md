...
9. [常见问题解决](#9-常见问题解决)
10. [WiderFace 人脸检测训练](#10-widerface-人脸检测训练)
    - [10.1 准备 WiderFace 数据集](#101-准备-widerface-数据集)
    - [10.2 模型训练](#102-模型训练)
    - [10.3 推理与评估](#103-推理与评估)
    - [10.4 完整训练流程](#104-完整训练流程)
11. [进阶使用技巧](#11-进阶使用技巧)
...
本节介绍如何使用 YOLOv13 在 WiderFace 人脸检测数据集上进行训练、推理和评估。

### 10.1 准备 WiderFace 数据集

#### 10.1.1 下载原始数据

WiderFace 数据集可以从官网下载：http://shuoyang1213.me/WIDERFACE/
...
WiderFace 官方评估需要使用专用工具：

```bash
# 克隆评估工具仓库
git clone https://github.com/wondervictor/WiderFace-Evaluation widerface_evaluate
...
运行 `prepare_data.py` 脚本将 WiderFace 原始标注转换为 YOLO 格式：
...
```python
# prepare_data.py 顶部配置
WIDERFACE_TRAIN_DIR = 'E:/biyesheji/datasets/widerface/original/train'
WIDERFACE_VAL_DIR   = 'E:/biyesheji/datasets/widerface/original/val'
SAVE_TRAIN_DIR      = 'datasets/widerface/train'
SAVE_VAL_DIR        = 'datasets/widerface/val'
```
...
| 参数 | 默认值 | 说明 |
|------|--------|------|
| MODEL | yolov13n.pt | 预训练权重（n/s/m/l/x） |
| EPOCHS | 300 | 训练轮数 |
| BATCH_SIZE | 16 | 批量大小 |
| IMG_SIZE | 640 | 输入图像尺寸 |
| DEVICE | 0 | GPU 编号 |
| NAME | yolov13n_widerface | 实验名称 |

训练完成后，权重保存在 `runs/train/yolov13n_widerface/weights/best.pt`
...
|------|------|
| --weights | 训练好的权重路径 |
| --dataset_folder | WiderFace val 图片目录 |
| --save_folder | 预测结果保存目录 |
| --img-size | 推理图像尺寸（默认 640） |
| --conf-thres | 置信度阈值（默认 0.001） |
| --iou-thres | NMS IoU 阈值（默认 0.5） |

评估完成后会输出 Easy / Medium / Hard 三个难度的 AP 值。
...
第一次使用，先准备评估工具（只需一次）：
  git clone https://github.com/wondervictor/WiderFace-Evaluation widerface_evaluate
  cd widerface_evaluate && python setup.py build_ext --inplace && cd ..

然后按顺序运行三个脚本：

  python prepare_data.py       ← 数据转换（只需一次）
         ↓
  python train_widerface.py    ← 训练
         ↓
  python test_widerface.py     ← 推理 + 评估，输出 AP
```

每次只需要改对应脚本顶部的路径变量，其余不用动。
